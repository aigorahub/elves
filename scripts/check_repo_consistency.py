...
def main() -> int:
    ...
    for label, phrases in LEARNINGS_LEDGER_PHRASES.items():
        path = REPO_ROOT / label
        text = read_text(path)
        for phrase in phrases:
            if phrase not in text:
    ...
    for label, phrases in SALVAGE_PREVIEW_PHRASES.items():
        path = REPO_ROOT / label
        text = read_text(path)
        for phrase in phrases:
            if phrase not in text:
    ...
    errors.extend(
        find_missing_phrases(
            {
                label: read_text(REPO_ROOT / label)
                for label in FUGU_INVOCATION_GRAMMAR_PHRASES
            },
            FUGU_INVOCATION_GRAMMAR_PHRASES,
            "Fugu invocation grammar",
        )
    )
    errors.extend(
        find_missing_phrases(

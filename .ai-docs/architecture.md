...
digests; executable profile checks are rejected. `scripts/cobbler_runtime/landing_profile_learn.py`
owns observation/candidate/waiver runtime state and explicit promotion into the tracked profile.
`scripts/landing_profile.py` is the installed CLI; `elves_landing_check.py` recomputes and binds the
live result to distinct host-owned readiness state that worker reports cannot override. Profiles may
block but never grant authority. Learning never auto-promotes. Same-user worker isolation remains
the inherited landing-authority protocol trust model, not a profile sandbox.

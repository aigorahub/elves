...
Every plan is written for an executor model that has **zero context**: it has not seen the discovery session, the audit, the other plans, or any prior conversation. It may be a smaller/cheaper model. Assume it is competent at following explicit instructions and weak at filling gaps, recovering from ambiguity, or knowing when to stop.
...
File naming: `advisor-plans/NNN-short-slug.md`, numbered in recommended execution order.
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: advisor-plans/NNN-*.md (or "none")
- **Category**: bug | security | perf | tests | tech-debt | migration | dx | docs | direction
- **Planned at**: commit `<short SHA>`, <YYYY-MM-DD>
- **Issue**: <GitHub issue URL — only when the finding was filed with `gh issue create`; omit otherwise>
...
## Index file: `advisor-plans/README.md`

Written once by the discovery pass after all plans, updated by executors:

```markdown
# Implementation Plans

Generated by an Elves discovery pass on <date>. Execute in the order below unless
...
- Could a model that has never seen this repo execute this with only the plan file and the repo? If any step requires knowledge from the discovery session, inline that knowledge.

...
- General Fugu task → `run_fugu.sh [--deep|--ultra] [--write] [--include PATH] <task>`; explicit
  ...
  paths, unsafe file types and links, and repository escapes. `--write` requires independent user
  implementation authority, writes only the disposable kernel-isolated snapshot, and returns an
  audited inert handoff that is never applied automatically. No Linux procfs is mounted; Codex
  external-sandbox mode runs inside the mandatory outer boundary. Profiles remain `fugu/high`,
  `fugu/xhigh` for `--deep`, and `fugu-ultra/high` for `--ultra`; Ultra uses exact-session staged
  synthesis.
